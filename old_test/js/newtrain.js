(function ($container) {
    ...
    function initTabs () {
        ...
        $tabs
            .on('click', function () {
                ...
                if (
                    'two_sets_association' == $currentQuestion.attr('data-interaction-type') ||
                    'inline_choice' == $currentQuestion.attr('data-interaction-type')
                ) {
    ...
    function finish (e) {
        ...
        if ($container.attr('data-for-check-by-expert')) {
            postData['check_by_expert'] = true;
        }

        $.post($container.attr('data-attempt-url'), postData, function (data) {
            if (true == data.success) {
                if (data.btnText) {
                    if (data.btnText == 'Изучите материал урока') {
                        swal({
                        ...
                        swal({
                            title: "Вы уже успешно прошли урок!",
                            text: ""
                        });
                    }
                } else {
